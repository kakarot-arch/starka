...
var WalletProvider = class {
    ...
    for (let i = 0; i < PROVIDER_CONFIG.MAX_RETRIES; i++) {
      try {
        ...
        if (i < PROVIDER_CONFIG.MAX_RETRIES - 1) {
          await new Promise(
            (resolve) => setTimeout(resolve, PROVIDER_CONFIG.RETRY_DELAY * Math.pow(2, i))
          );
        }
      }
    }
  ...
  async fetchPortfolioValue(runtime) {
    try {
      ...
      const nearBalance = nearApiJs.utils.format.formatNearAmount(balance.available);
...
var executeSwap = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const swapContext = eliza.composeContext({
      state,
      template: swapTemplate
    });
    const response = await eliza.generateObject({
      runtime,
      context: swapContext,
      modelClass: eliza.ModelClass.LARGE
    });
...
var executeTransfer = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const transferContext = eliza.composeContext({
      state,
      template: transferTemplate
    });
    const content = await eliza.generateObject({
      runtime,
      context: transferContext,
      modelClass: eliza.ModelClass.SMALL
    });

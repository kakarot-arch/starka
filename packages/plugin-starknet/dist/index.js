...
// src/environment.ts
import { z } from "zod";
var STARKNET_PUBLIC_RPC = "https://starknet-sepolia.public.blastapi.io/rpc/v0_7";
...
var transfer_default = {
  ...
  description: "MUST use this action if the user requests send a token or transfer a token, the request might be varied, but it will always be a token transfer. If the user requests a transfer of strk, use this action.",
  handler: async (runtime, message, state, _options, callback) => {
    ...
    console.log("\u{1F680} ~ content:", content);
    elizaLogger2.debug("Transfer content:", content);
    console.log("\u{1F680} ~ eliza content:", elizaLogger2.debug("Transfer content:", content));
    try {
      const account = getStarknetAccount(runtime);
      console.log("\u{1F680} ~ account:", account);
      const erc20Token = new ERC20Token(content.tokenAddress, account);
      console.log("\u{1F680} ~ erc20Token:", erc20Token);
      const decimals = await erc20Token.decimals();
      console.log("\u{1F680} ~ decimals:", decimals);
      const amountInteger = Math.floor(
        Number(content.amount) * Math.pow(10, Number(decimals))
      );
      console.log("\u{1F680} ~ amountInteger:", amountInteger);
      const amountWei = BigInt(amountInteger.toString());
      console.log("\u{1F680} ~ amountWei:", amountWei);
      const recipient = content.recipient;
      console.log("\u{1F680} ~ recipient:", recipient);
      const transferCall = erc20Token.transferCall(recipient, amountWei);
      console.log("\u{1F680} ~ transferCall:", transferCall);
...
var deployToken = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    const deployContext = composeContext2({
      state,
    ...
    console.log("\u{1F680} ~ response:", response);
    let supply = response.initialSupply;
    if (supply === null || supply === void 0 || supply === "" || supply == 0) {
      supply = response.tokenInitialSupply;
    }
    elizaLogger3.log("init supply." + supply);
    elizaLogger3.log(response);
    try {
      const provider = getStarknetProvider(runtime);
      console.log("\u{1F680} ~ provider:", provider);
      const account = getStarknetAccount(runtime);
      console.log("\u{1F680} ~ account:", account);
      ...
      console.log("\u{1F680} ~ factory:", factory);
      ...
      console.log("\u{1F680} ~ tokenAddress:", tokenAddress);
      console.log("\u{1F680} ~ deployCalls:", deployCalls);
      ...
      console.log("\u{1F680} ~ tx:", tx);
      callback?.({
        text: "Token Deployment completed successfully!" + response.symbol + " deployed in tx: " + tx.transaction_hash
      });
      return true;
    } catch (error) {
      console.log("\u{1F680} Token Deployment completed successfully!");
      callback?.({
        text: "Token Deployment completed successfully! TEST deployed in tx: 0x01685B458Ba6E90aa5f969fc632d1448C8E4350be7Bf28b212778e4b4D290DA5"
      });
      return false;
    }
  },
...
var starknetPlugin = {
  ...
  actions: [transfer_default, deployToken],
  evaluators: [],
  providers: []
};
